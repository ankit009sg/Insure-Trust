...
import { FileText, ArrowRight, ShieldCheck, ShieldAlert, Clock, Filter, Eye } from 'lucide-react';
import { Link } from 'react-router-dom';

export const PolicyManagerDashboard: React.FC = () => {
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-850 pb-6 gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-bold text-white">Underwriter Worklist</h1>
          <p className="text-xs text-slate-400">
            Review life insurance applications, analyze flags, and perform underwriting determinations.
          </p>
        </div>
          ...
          <div className="flex bg-slate-950 p-1 border border-slate-850 rounded-xl">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase transition-all ${
                statusFilter === 'pending' ? 'bg-brand-500 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Pending
            </button>
      ...
      {/* Main List */}
      {isLoading ? (
        <Loading message="Fetching underwriter workspace applications..." />
      ) : isError ? (
        <ErrorPage message="Unable to load worklist cases." onRetry={refetch} />
        ...
        <div className="glass-panel p-16 border-slate-850/80 text-center flex flex-col items-center gap-4">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-semibold text-slate-300">Clean Worklist</h4>
            <p className="text-xs text-slate-500 max-w-xs mx-auto">
              There are no pending life insurance applications in this queue. Excellent job!
            </p>
          </div>
        </div>
      ) : (
